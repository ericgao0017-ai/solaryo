...
async function sendFinalEnquiry() {
    ...
    const postcode = document.getElementById('conf-postcode').value;
    const timeframe = document.getElementById('conf-timeframe').value;
    ...
    if (!isValidPostcode(postcode)) {
        document.getElementById('final-msg').style.color = 'red';
        document.getElementById('final-msg').innerText = i18n[curLang].err_postcode;
        return;
    }
    if (file && file.size > 5 * 1024 * 1024) {
        document.getElementById('final-msg').style.color = 'red';
        document.getElementById('final-msg').innerText = curLang === 'cn' ? "文件过大，请上传小于 5MB 的文件" : "File too large (Max 5MB)";
        return;
    }
    ...
    try {
        ...
        if (file) { fileData = await readFileAsBase64(file); fileName = file.name; }

        const finalPayload = {
            timestamp: new Date().toISOString(),
            contact: { name, phone, email: document.getElementById('conf-email').value, postcode, method: contactMethod ? contactMethod.value : 'phone', timeframe },
            requirements: { notes: notes, hasAttachment: !!file, fileName: fileName, userProfile: userApplianceProfile },
            systemConfig: { type: curMode, solarSize: document.getElementById('solar-val').innerText, batterySize: document.getElementById('bat-val').innerText, tier: selectedTier, bill: document.getElementById('bill-input').value }
        };

        console.log("🚀 Final Enquiry Payload:", finalPayload);
        ...
        document.getElementById('final-msg').style.color = 'red';
        document.getElementById('final-msg').innerText = "Error processing file.";
        btn.disabled = false;
